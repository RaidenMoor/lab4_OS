...
// Структура для многопоточного стека
...
DWORD WINAPI ConsumerThread(LPVOID lpParam) {
    ...
    while (true) {
        // Ожидаем либо данные, либо команду остановки
...
// Реализация C-интерфейса для базового стека
extern "C" {
...
// Реализация многопоточных функций
extern "C" {
    STACK_API ThreadedStackHandle CreateThreadedStack() {
        ...
        // Создание событий
        data->hDataReadyEvent = CreateEvent(NULL, FALSE, FALSE, NULL);    // Auto-reset
        data->hDataProcessedEvent = CreateEvent(NULL, FALSE, FALSE, NULL); // Auto-reset
        data->hStopEvent = CreateEvent(NULL, TRUE, FALSE, NULL);          // Manual-reset
    ...
    STACK_API void ThreadedPush(ThreadedStackHandle handle, int value) {
        ...
        // Ждем обработки данных (опционально, для синхронизации)
...
// Реализации методов Stack (без изменений)
template <typename T>
